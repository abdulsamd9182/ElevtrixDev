...
import { GraduationCap, ShoppingBag, HeartPulse, Truck, Banknote, Sparkles } from "lucide-react";
import { motion } from "framer-motion";
import { useEffect, useRef, useState } from "react";

const industries = [
  { name: "E-Commerce", description: "Regression test suites that validate checkout flows, payment integrations, and complex cart logic on every code push.", icon: ShoppingBag, color: "from-cyan-500 to-blue-500", stats: "25+ Frameworks Built" },
  { name: "Healthcare", description: "Automated validation of patient workflows, EHR integrations, and form logic ensuring consistent behavior across releases.", icon: HeartPulse, color: "from-purple-500 to-pink-500", stats: "10+ Suites" },
  { name: "EdTech", description: "Automated testing of LMS workflows, video delivery, quiz scoring engines, and multi-role permission logic.", icon: GraduationCap, color: "from-green-500 to-emerald-500", stats: "12+ Projects" },
  { name: "Logistics", description: "End-to-end automation of tracking UIs, invoice generation, and driver app workflows across device matrices.", icon: Truck, color: "from-orange-500 to-yellow-500", stats: "8+ Systems" },
  { name: "FinTech", description: "Precision automation of transaction flows, reporting dashboards, and regulatory compliance pages with zero tolerance for failures.", icon: Banknote, color: "from-indigo-500 to-blue-500", stats: "15+ Platforms" }
];
...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    let animId: number;
    const animate = () => { ctx.clearRect(0, 0, width, height); for (let p of particles) { p.x += p.speedX; p.y += p.speedY; if (p.x < 0) p.x = width; if (p.x > width) p.x = 0; if (p.y < 0) p.y = height; if (p.y > height) p.y = 0; ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, 2 * Math.PI); ctx.fillStyle = `rgba(6, 182, 212, ${p.opacity})`; ctx.fill(); } animId = requestAnimationFrame(animate); };
    animate();
              ...
              {[...industries, ...industries].map((item, idx) => (
                <motion.div key={`${item.name}-${idx}`} whileHover={{ y: -8 }} className="group relative w-[320px] min-h-[300px] flex flex-col">
                  ...
                  <TiltCard className="relative p-8 bg-white border border-gray-100 rounded-[2.5rem] hover:border-transparent transition-all duration-500 shadow-lg hover:shadow-2xl h-full flex flex-col">
                    ...
                    <motion.h3 whileHover={{ scale: 1.05 }} className="text-base font-bold text-gray-900 mb-3 text-center group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-600 group-hover:to-blue-600 transition-all duration-300">{item.name}</motion.h3>
                    <p className="text-gray-500 text-xs leading-relaxed font-normal text-center mb-6 flex-grow">{item.description}</p>
                    <div className="flex items-center justify-center gap-1 mt-auto"><span className="text-[10px] font-semibold text-gray-400">{item.stats}</span><Sparkles className="w-3 h-3 text-cyan-400" /></div>
                  </TiltCard>
                </motion.div>
              ))}
            </motion.div>
