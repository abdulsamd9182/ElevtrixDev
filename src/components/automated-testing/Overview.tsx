...
import { Play, GitBranch, Code2, Gauge } from "lucide-react";
import { motion } from "framer-motion";
import { useEffect, useRef, useState } from "react";

const features = [
  { title: "CI/CD Integration", description: "Every pull request triggers your full test suite automatically, catching regressions before the merge ever lands in main.", icon: GitBranch, color: "from-blue-500 to-cyan-500" },
  { title: "Framework Engineering", description: "We build maintainable Page Object Models in TypeScript or Python — not fragile, brittle scripts.", icon: Code2, color: "from-purple-500 to-pink-500" },
  { title: "Fast Feedback Loops", description: "Parallel execution strategies that reduce your 2-hour test run to under 10 minutes.", icon: Gauge, color: "from-yellow-500 to-orange-500" },
  { title: "Reliable Coverage", description: "Deterministic, flake-free tests that your team trusts. We eliminate false positives that erode confidence.", icon: Play, color: "from-green-500 to-emerald-500" },
];
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 40; i++) { particles.push({ x: Math.random() * width, y: Math.random() * height, size: 1 + Math.random() * 2.5, speedX: (Math.random() - 0.5) * 0.1, speedY: (Math.random() - 0.5) * 0.1, opacity: 0.1 + Math.random() * 0.2, hue: 180 + Math.random() * 40 }); }
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-24 bg-white overflow-hidden">
      ...
      <div className="absolute top-20 left-20 w-96 h-96 bg-cyan-50/30 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-20 right-20 w-80 h-80 bg-blue-50/30 rounded-full blur-3xl pointer-events-none" />
      <div className="relative max-w-7xl mx-auto px-6 md:px-16 z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0, transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="lg:col-span-5 text-center lg:text-left flex flex-col items-center lg:items-start">
            <div className="text-3xl md:text-4xl font-bold tracking-tight text-gray-900 leading-[1.2] mb-6">DEPLOY WITH <br /><span className="text-cyan-600">CONFIDENCE</span></div>
            <p className="text-gray-500 text-base md:text-lg leading-relaxed mb-8 max-w-lg">Automation isn't just about speed — it's about building a safety net that lets your team innovate fearlessly, knowing every regression is caught instantly.</p>
            <div className="flex gap-6 justify-center lg:justify-start"><Counter value="85" label="Faster Releases" /><Counter value="92" label="Coverage Rate" /></div>
          </motion.div>
