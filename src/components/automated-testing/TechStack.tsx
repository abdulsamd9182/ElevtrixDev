...
import { Terminal, Globe, GitBranch, Code2, Sparkles, Cpu, Gauge } from "lucide-react";
import { useEffect, useRef, useState } from "react";

const stacks = [
  { category: "E2E Frameworks", items: ["Playwright", "Cypress", "Selenium WebDriver", "Puppeteer"], icon: Globe, color: "from-cyan-500 to-blue-500", expertise: 94, description: "Engineering reliable end-to-end automation frameworks using the modern browser automation tools, with a strong preference for Playwright for its superior speed and multi-browser capabilities.", technologies: ["Playwright (TypeScript)", "Cypress", "Selenium 4", "Puppeteer"], stats: { projects: "45+ Suites", experience: "5+ years" } },
  { category: "API Testing", items: ["Postman", "Newman", "REST Assured", "Pactflow"], icon: Terminal, color: "from-green-500 to-emerald-500", expertise: 91, description: "Contract and integration testing of REST and GraphQL APIs using Newman for CI pipeline automation and Pactflow to implement consumer-driven contract testing between microservices.", technologies: ["Postman/Newman", "REST Assured", "Pactflow", "k6 API"], stats: { projects: "38+ APIs Covered", experience: "4+ years" } },
  { category: "CI/CD & Reporting", items: ["GitHub Actions", "Jenkins", "GitLab CI", "Allure Reports"], icon: GitBranch, color: "from-purple-500 to-pink-500", expertise: 89, description: "Integrating test pipelines into your CI systems with Allure dashboards for rich historic reporting. We configure fail-fast policies, Slack notifications, and parallel execution across cloud agents.", technologies: ["GitHub Actions", "Jenkins Pipelines", "Allure", "GitLab CI/CD"], stats: { projects: "30+ Pipelines", experience: "4+ years" } }
];
...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 60; i++) { const x = Math.random() * width; const y = Math.random() * height; particles.push({ x, y, baseX: x, baseY: y, size: 1 + Math.random() * 4, speedX: (Math.random() - 0.5) * 0.2, speedY: (Math.random() - 0.5) * 0.2, opacity: 0.1 + Math.random() * 0.3, hue: 180 + Math.random() * 60 }); }
    let animId: number;
    const animate = () => {
...
function TiltCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return <motion.div ref={ref} onMouseMove={handleMouseMove} onMouseLeave={() => { setRotateX(0); setRotateY(0); }} animate={{ rotateX, rotateY }} transition={{ type: "spring", stiffness: 300, damping: 25 }} style={{ transformStyle: "preserve-3d" }} className={className}>
    <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: `radial-gradient(circle at ${glow.x}% ${glow.y}%, rgba(6,182,212,0.15) 0%, transparent 50%)` }} />
...
export default function TechStack() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-24 bg-white overflow-hidden">
      <ParticleField />
      <motion.div animate={{ background: `radial-gradient(circle at ${mousePos.x}% ${mousePos.y}%, rgba(6,182,212,0.08) 0%, transparent 60%)` }} transition={{ type: "spring", stiffness: 30, damping: 20 }} className="absolute inset-0 pointer-events-none" />
      <motion.div animate={{ x: [0, 50, 0], y: [0, -40, 0], scale: [1, 1.3, 1] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} className="absolute top-0 right-0 w-[600px] h-[600px] bg-cyan-50/40 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4 pointer-events-none" />
      <motion.div animate={{ x: [0, -50, 0], y: [0, 50, 0], scale: [1, 1.3, 1] }} transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-blue-50/40 rounded-full blur-[100px] translate-y-1/4 -translate-x-1/4 pointer-events-none" />
      <div className="relative max-w-6xl mx-auto px-6 md:px-8 z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { staggerChildren: 0.1, duration: 0.8 } } }} className="text-center mb-16">
          <motion.h2 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-2xl md:text-3xl lg:text-4xl font-bold tracking-tight text-gray-900">TOOLS & <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600">STACK</span></motion.h2>
          <motion.p variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { duration: 0.8 } } }} className="text-gray-500 text-lg max-w-2xl mx-auto mt-4">Best-in-class automation tooling for maximum coverage, speed, and reliability</motion.p>
        </motion.div>
        <div className="relative">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-200 via-blue-200 to-indigo-200 hidden md:block" />
                  ...
                  <div className={`${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'} flex justify-${index % 2 === 0 ? 'end' : 'start'}`}>
                    <div className="md:sticky md:top-24 w-full max-w-sm">
                      <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-4">
                        <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${stack.color} flex items-center justify-center shadow-2xl shadow-cyan-500/20 group flex-shrink-0`}>
                        ...
                        <div><h3 className="text-xl font-bold text-gray-900">{stack.category}</h3><div className="flex items-center gap-2 mt-1"><Gauge className="w-3.5 h-3.5 text-cyan-500" /><span className="text-xs text-gray-400">Expertise {stack.expertise}%</span></div></div>
                      </motion.div>
                    </div>
                  </div>
                    ...
                    <TiltCard>
                      <motion.div whileHover={{ y: -4 }} className="relative p-6 bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl hover:border-transparent transition-all duration-500 shadow-lg hover:shadow-xl group overflow-hidden">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="flex gap-3 mb-4"><div className="flex items-center gap-1"><Cpu className="w-3 h-3 text-cyan-500" /><span className="text-[10px] text-gray-500">{stack.stats.projects}</span></div><div className="flex items-center gap-1"><Sparkles className="w-3 h-3 text-cyan-500" /><span className="text-[10px] text-gray-500">{stack.stats.experience}</span></div></div>
                          <div className="mb-4"><h4 className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-2 flex items-center gap-2"><span className="w-1 h-2 bg-cyan-500 rounded-full" />KEY TECH</h4><div className="flex flex-wrap gap-1.5">{stack.technologies.map(tech => (<motion.span key={tech} whileHover={{ scale: 1.05, y: -2 }} className="px-2 py-1 bg-gray-50 border border-gray-100 rounded-full text-[9px] font-medium text-gray-600 hover:bg-gradient-to-r hover:from-cyan-600 hover:to-blue-600 hover:text-white hover:border-transparent transition-all duration-200 cursor-default shadow-sm">{tech}</motion.span>))}</div></div>
