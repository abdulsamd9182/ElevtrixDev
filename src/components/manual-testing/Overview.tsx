...
import { Search, MonitorSmartphone, Bug, Users } from "lucide-react";
...
const features = [
  {
    ...
    description: "Unscripted, creative testing where engineers actively learn and execute to find edge cases missed by automation.",
    icon: Search,
    color: "from-blue-500 to-cyan-500",
  },
  {
    title: "Cross-Device Testing",
    description: "Ensuring pixel-perfect consistency across hundreds of real devices, tablets, and browsers.",
    icon: MonitorSmartphone,
    color: "from-purple-500 to-pink-500",
  },
  {
    title: "Defect Identification",
    description: "Precise reproduction steps and detailed bug reports linked directly to your development sprints.",
    icon: Bug,
    color: "from-yellow-500 to-orange-500",
  },
  {
    title: "Usability Assessment",
    description: "Evaluating intuitiveness and UX from a real human perspective — something no script can replicate.",
    icon: Users,
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 40; i++) {
      particles.push({ x: Math.random() * width, y: Math.random() * height, size: 1 + Math.random() * 2.5, speedX: (Math.random() - 0.5) * 0.1, speedY: (Math.random() - 0.5) * 0.1, opacity: 0.1 + Math.random() * 0.2, hue: 200 + Math.random() * 40 });
    }
    let time = 0;
    let animId: number;
    const animate = () => {
      ...
      for (let p of particles) {
        if (mousePos.x && mousePos.y) {
          const dx = p.x - mousePos.x; const dy = p.y - mousePos.y;
          const dist = Math.sqrt(dx * dx + dy * dy);
          if (dist < 80) { const angle = Math.atan2(dy, dx); const force = (80 - dist) / 80 * 0.3; p.x += Math.cos(angle) * force * 3; p.y += Math.sin(angle) * force * 3; }
        }
        ...
        ctx.fillStyle = `hsla(${p.hue}, 60%, 70%, ${p.opacity})`; ctx.fill();
      }
      time += 0.005;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          ...
          >
            <div className="text-3xl md:text-4xl font-bold tracking-tight text-gray-900 leading-[1.2] mb-6">
              THE HUMAN <br />
              <span className="text-blue-600">QUALITY LAYER</span>
            </div>
          ...
          >
            {features.map((feature) => (
              <motion.div
                ...
                className="group relative p-6 bg-white border border-gray-100 rounded-2xl shadow-sm hover:border-blue-100 hover:shadow-[0_20px_60px_-15px_rgba(59,130,246,0.2)] transition-all duration-500"
              >
