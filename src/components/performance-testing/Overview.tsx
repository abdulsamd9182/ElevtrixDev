...
import { GaugeCircle, Gauge, Network, AlertTriangle } from "lucide-react";
import { motion } from "framer-motion";
import { useEffect, useRef, useState } from "react";

const features = [
  { title: "Architectural Limits", description: "Discover exactly where and how your application breaks — whether it's the database tier, API gateway, or worker memory exhaustion.", icon: Network, color: "from-blue-500 to-cyan-500" },
  { title: "Zero Downtime Scaling", description: "Ensure auto-scaling rules trigger correctly and user experience remains stable while new instances spin up during high traffic.", icon: GaugeCircle, color: "from-purple-500 to-pink-500" },
  { title: "SLA Compliance", description: "Mathematically prove to enterprise clients that you maintain 99.99% uptime and sub-200ms API response times under target loads.", icon: Gauge, color: "from-yellow-500 to-orange-500" },
  { title: "Disaster Avoidance", description: "A crashed site during a major campaign causes devastating revenue loss. Load testing prevents this before it reaches production.", icon: AlertTriangle, color: "from-green-500 to-emerald-500" },
];
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 40; i++) { particles.push({ x: Math.random() * width, y: Math.random() * height, size: 1 + Math.random() * 2.5, speedX: (Math.random() - 0.5) * 0.1, speedY: (Math.random() - 0.5) * 0.1, opacity: 0.1 + Math.random() * 0.2, hue: 150 + Math.random() * 40 }); }
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-24 bg-white overflow-hidden">
      ...
      <div className="absolute top-20 left-20 w-96 h-96 bg-emerald-50/30 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-20 right-20 w-80 h-80 bg-teal-50/30 rounded-full blur-3xl pointer-events-none" />
      <div className="relative max-w-7xl mx-auto px-8 md:px-16 z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0, transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="lg:col-span-5">
            <div className="text-3xl md:text-4xl font-bold tracking-tight text-gray-900 leading-[1.2] mb-6">WHY ARCHITECTURE <br /><span className="text-emerald-600">BREAKS</span></div>
            <p className="text-gray-500 text-base md:text-lg leading-relaxed mb-8 max-w-lg">Your code might handle 10 users perfectly but collapse at 10,000. We find the race conditions, memory leaks, and deadlocks that only manifest under immense pressure.</p>
            <div className="flex gap-6"><Counter value="99" label="Uptime Proven" /><Counter value="60" label="Faster Response" /></div>
          </motion.div>
