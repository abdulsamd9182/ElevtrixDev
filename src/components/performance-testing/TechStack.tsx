...
import { Activity, Server, BarChart2, Sparkles, Cpu, Gauge } from "lucide-react";
import { useEffect, useRef, useState } from "react";

const stacks = [
  { category: "Load Generators", items: ["Apache JMeter", "k6", "Gatling", "Locust"], icon: Activity, color: "from-emerald-500 to-teal-500", expertise: 93, description: "Generating realistic user loads using industry-standard tools like k6 and JMeter. We write sophisticated load scripts that accurately model real user behaviors including think times, pacing, and correlations.", technologies: ["k6 (JavaScript)", "Apache JMeter", "Gatling (Scala)", "Locust (Python)"], stats: { projects: "40+ Tests Run", experience: "5+ years" } },
  { category: "APM & Monitoring", items: ["Grafana", "New Relic", "Datadog", "Prometheus"], icon: Server, color: "from-blue-500 to-indigo-500", expertise: 90, description: "Correlating synthetic load data with real server-side metrics from APM tools to pinpoint whether bottlenecks originate in application code, the database, or external API dependencies.", technologies: ["Grafana", "Prometheus", "Datadog APM", "New Relic"], stats: { projects: "30+ Environments", experience: "4+ years" } },
  { category: "Profiling & Analysis", items: ["Flame Graphs", "async-profiler", "Heap Dump", "Thread Analysis"], icon: BarChart2, color: "from-purple-500 to-pink-500", expertise: 85, description: "Deep-diving into JVM heap dumps, thread dumps, and CPU flame graphs to identify the exact line of code causing memory leaks or thread contention under concurrent load.", technologies: ["async-profiler", "JVM Profiling", "Flame Graphs", "Memory Analyzers"], stats: { projects: "25+ Root Causes Found", experience: "4+ years" } }
];
...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 60; i++) { const x = Math.random() * width; const y = Math.random() * height; particles.push({ x, y, baseX: x, baseY: y, size: 1 + Math.random() * 4, opacity: 0.1 + Math.random() * 0.3, hue: 150 + Math.random() * 60 }); }
    let animId: number;
    const animate = () => {
...
function TiltCard({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return <motion.div ref={ref} onMouseMove={handleMouseMove} onMouseLeave={() => { setRotateX(0); setRotateY(0); }} animate={{ rotateX, rotateY }} transition={{ type: "spring", stiffness: 300, damping: 25 }} style={{ transformStyle: "preserve-3d" }} className={className}>
    <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: `radial-gradient(circle at ${glow.x}% ${glow.y}%, rgba(16,185,129,0.15) 0%, transparent 50%)` }} />
...
export default function TechStack() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-24 bg-white overflow-hidden">
      <ParticleField />
      <motion.div animate={{ background: `radial-gradient(circle at ${mousePos.x}% ${mousePos.y}%, rgba(16,185,129,0.08) 0%, transparent 60%)` }} transition={{ type: "spring", stiffness: 30, damping: 20 }} className="absolute inset-0 pointer-events-none" />
      <motion.div animate={{ x: [0, 50, 0], y: [0, -40, 0], scale: [1, 1.3, 1] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} className="absolute top-0 right-0 w-[600px] h-[600px] bg-emerald-50/40 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4 pointer-events-none" />
      <motion.div animate={{ x: [0, -50, 0], y: [0, 50, 0], scale: [1, 1.3, 1] }} transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-50/40 rounded-full blur-[100px] translate-y-1/4 -translate-x-1/4 pointer-events-none" />
      <div className="relative max-w-6xl mx-auto px-6 md:px-8 z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { staggerChildren: 0.1, duration: 0.8 } } }} className="text-center mb-16">
          <motion.h2 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-2xl md:text-3xl lg:text-4xl font-bold tracking-tight text-gray-900">TOOLS & <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-teal-600 to-green-600">STACK</span></motion.h2>
          <motion.p variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { duration: 0.8 } } }} className="text-gray-500 text-lg max-w-2xl mx-auto mt-4">Industry-leading load generation and observability tools we use to find performance bottlenecks</motion.p>
        </motion.div>
        <div className="relative">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-emerald-200 via-teal-200 to-green-200 hidden md:block" />
          <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.18, delayChildren: 0.2 } } }} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="space-y-16">
            {stacks.map((stack, index) => (
              <motion.div key={stack.category} className="relative" variants={{ hidden: { opacity: 0, y: 50 }, visible: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 80, damping: 12 } } }}>
                <div className="absolute left-1/2 -translate-x-1/2 w-5 h-5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 hidden md:flex items-center justify-center z-10 shadow-lg shadow-emerald-500/30"><div className="w-2 h-2 rounded-full bg-white animate-pulse" /></div>
                  ...
                  <div className={`${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'} flex justify-${index % 2 === 0 ? 'end' : 'start'}`}>
                    <div className="md:sticky md:top-24 w-full max-w-sm">
                      <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-4">
                        <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${stack.color} flex items-center justify-center shadow-2xl shadow-emerald-500/20 group flex-shrink-0`}>
                        ...
                        <div><h3 className="text-xl font-bold text-gray-900">{stack.category}</h3><div className="flex items-center gap-2 mt-1"><Gauge className="w-3.5 h-3.5 text-emerald-500" /><span className="text-xs text-gray-400">Expertise {stack.expertise}%</span></div></div>
                      </motion.div>
                    </div>
                  </div>
                    ...
                    <TiltCard>
                      <motion.div whileHover={{ y: -4 }} className="relative p-6 bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl hover:border-transparent transition-all duration-500 shadow-lg hover:shadow-xl group overflow-hidden">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="flex gap-3 mb-4"><div className="flex items-center gap-1"><Cpu className="w-3 h-3 text-emerald-500" /><span className="text-[10px] text-gray-500">{stack.stats.projects}</span></div><div className="flex items-center gap-1"><Sparkles className="w-3 h-3 text-emerald-500" /><span className="text-[10px] text-gray-500">{stack.stats.experience}</span></div></div>
                          <div className="mb-4"><h4 className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-2 flex items-center gap-2"><span className="w-1 h-2 bg-emerald-500 rounded-full" />KEY TECH</h4><div className="flex flex-wrap gap-1.5">{stack.technologies.map(tech => (<motion.span key={tech} whileHover={{ scale: 1.05, y: -2 }} className="px-2 py-1 bg-gray-50 border border-gray-100 rounded-full text-[9px] font-medium text-gray-600 hover:bg-gradient-to-r hover:from-emerald-600 hover:to-teal-600 hover:text-white hover:border-transparent transition-all duration-200 cursor-default shadow-sm">{tech}</motion.span>))}</div></div>
